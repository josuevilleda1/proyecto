    ...
    public int saturacionElevadores(ElevatorInterface elevador, peticionusuario solicitud) {
        ...
        if (DireccionDelElevador == dondeSeLLamo) {
            if (DireccionDelElevador == Direction.UP && pisoActual <= pisoSolicitud) {
                ...
                puntuacion += (pisos - (pisoSolicitud - pisoActual)); // Más puntos si está más cerca
            }
        ...
        // REGLA 2: Elevador inactivo o con cola vacía
        if (elevador.getListCopy().isEmpty()) {
            puntuacion += 50;
            puntuacion += (pisos - Math.abs(pisoActual - pisoSolicitud)); // Puntos por cercanía
        }
        
        // REGLA 3: Penalizar cambio de dirección (REGLA OBLIGATORIA)
        if (DireccionDelElevador != dondeSeLLamo && DireccionDelElevador != null) {
            return 0; // No apto - no puede cambiar dirección
        }
        
        // REGLA 4: Penalizar elevadores muy ocupados
        int comandosPendientes = elevador.getListCopy().size();
        puntuacion -= (comandosPendientes * 10); // Menos puntos por cada comando pendiente
